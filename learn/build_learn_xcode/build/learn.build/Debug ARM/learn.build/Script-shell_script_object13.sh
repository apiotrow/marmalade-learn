...
if [ "$CONFIGURATION" = "Debug" -o "$CONFIGURATION" = "Release" -o "$CONFIGURATION" = "Debug ARM" -o "$CONFIGURATION" = "Release ARM" -o "$CONFIGURATION" = "Debug X86_64 NaCL" -o "$CONFIGURATION" = "Release X86_64 NaCL" ]; then echo "skipping for x86 and arm and NaCl builds" && exit 0; fi
...
s3e_plink -o "/Users/aaronpiotrowski/MarmaladeProjects/learn/learn/build_learn_xcode/build/${CONFIGURATION}${EFFECTIVE_PLATFORM_NAME}/learn.s3e" --arch=armv8-a --compiler=gcc -u -d --usemap="/Users/aaronpiotrowski/MarmaladeProjects/learn/learn/build_learn_xcode/build/${CONFIGURATION}${EFFECTIVE_PLATFORM_NAME}/learn.map" "${TARGET_BUILD_DIR}/${EXECUTABLE_NAME}"
