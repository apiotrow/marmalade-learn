...
if [ "$CONFIGURATION" = "Debug" -o "$CONFIGURATION" = "Release" -o "$CONFIGURATION" = "Debug X86_64 NaCL" -o "$CONFIGURATION" = "Release X86_64 NaCL" ]; then echo "skipping for x86 and NaCl builds" && exit 0; fi
...
s3e_plink -o "/Users/aaronpiotrowski/MarmaladeProjects/learn/learn/build_learn_xcode/build/${CONFIGURATION}${EFFECTIVE_PLATFORM_NAME}/learn.s3e" --arch=armv6 --compiler=gcc -u --usemap="/Users/aaronpiotrowski/MarmaladeProjects/learn/learn/build_learn_xcode/build/${CONFIGURATION}${EFFECTIVE_PLATFORM_NAME}/learn.map" "${TARGET_BUILD_DIR}/${EXECUTABLE_NAME}"
