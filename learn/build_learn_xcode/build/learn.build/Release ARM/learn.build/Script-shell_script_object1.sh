...
if [ "$CONFIGURATION" = "Debug ARM" -o "$CONFIGURATION" = "Release ARM" -o "$CONFIGURATION" = "Debug AARCH64" -o "$CONFIGURATION" = "Release AARCH64" -o "$CONFIGURATION" = "Debug X86_64 NaCL" -o "$CONFIGURATION" = "Release X86_64 NaCL" ]; then echo "skipping for arm and aarch64 and NaCl builds" && exit 0; fi
...
/usr/bin/python ./postlink.py "${TARGET_BUILD_DIR}/${EXECUTABLE_NAME}" "/Users/aaronpiotrowski/MarmaladeProjects/learn/learn/build_learn_xcode/build/${CONFIGURATION}${EFFECTIVE_PLATFORM_NAME}/learn.s86"
